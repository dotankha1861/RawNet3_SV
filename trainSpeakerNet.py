...
parser.add_argument('--n_dense',      type=int,   default=0,     help='# of dense adaptive network')
...
parser.add_argument('--freeze_model',    type=bool,   default=False,    help='')
...
def main_worker(gpu, ngpus_per_node, args):
    ...
    for it in range(it,args.max_epoch+1):
        ...
        if args.gpu == 0:
            print('\n',time.strftime("%Y-%m-%d %H:%M:%S"), "Epoch {:d}, TAcc {:2.2f}, TLOSS {:f}, LR {:f}".format(it, traineer, loss, max(clr)))
            scorefile.write("Epoch {:d}, TAcc {:2.2f}, TLOSS {:f}, LR {:f} \n".format(it, traineer, loss, max(clr)))
        ...
        if it % args.test_interval == 0:
            ...
            if args.gpu == 0:
                ...
                print('\n',time.strftime("%Y-%m-%d %H:%M:%S"), "Epoch {:d}, VEER {:2.4f}, VEER-thres {:2.3f}, MinDCF {:2.5f}, MinDCF-thres {:2.3f}".format(it, eer, eer_threshold, mindcf, mindcf_threshold))
                scorefile.write("Epoch {:d}, VEER {:2.4f}, VEER-thres {:2.3f}, MinDCF {:2.5f}, MinDCF-thres {:2.3f}\n".format(it, eer, eer_threshold, mindcf, mindcf_threshold))
